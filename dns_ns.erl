...
set(Name,Pid) when is_pid(Pid) ->
 case pid(Name) of    
  undefined -> R=global:register_name(Name,Pid),
               logger:critical(#{point => ?POINT,name => Name, pid => Pid, result => R, reason => "global:register name" }),
               Pid;
  
  OldPid       -> logger:critical(#{point => ?POINT, old_pid => OldPid, reason => "global already pid exists, new will be killed" }),
...
pids()-> lists:map(fun(N)-> {N,pid(dns_workers,N)} end, lists:seq(1,?WORKERS_NUM)).
