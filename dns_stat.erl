...
microservices()->
        #{ dnsd => dnsd:info(),
           workers => dns_workers:info(),
           queues  => dns_queue:info(),
           global_registered_names => global:registered_names()
         }.
...
by_global_names()->
 ...
 lists:foldl(fun(A,Acc)-> 
              maps:put(A,
                       erlang:process_info(try global:whereis_name(A) of R->R catch Err:Reason:Stack -> {Err,Reason,Stack} end),
                       Acc
                      ) 
             end,
             #{},
